...
FROM node:26.6.0-bookworm-slim AS web
WORKDIR /app
...
FROM node:26.6.0-bookworm-slim AS worker
WORKDIR /app
ENV NODE_ENV=production
COPY --from=builder /app/dist/worker.mjs ./worker.mjs
CMD ["node", "worker.mjs"]

FROM node:26.6.0-bookworm-slim AS bootstrap
WORKDIR /app
ENV NODE_ENV=production
...
FROM node:26.6.0-bookworm-slim AS evaluation
WORKDIR /app
