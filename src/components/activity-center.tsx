...
import { ArrowClockwise, Check, Clock, Database, Graph, HardDrives, Pulse, ShareNetwork, SpinnerGap, WarningCircle } from "@phosphor-icons/react";
...
export function ActivityCenter() {
  ...
  return <div className="page">
    <div className="page-heading"><div><div className="eyebrow">System pulse · observable</div><h1 className="page-title">Everything in <em>motion.</em></h1><p className="page-description">Follow documents from upload through parsing, indexing, and graph enrichment. Service checks update in the background.</p></div><div className="page-heading-meta">Health · live<br/>Queue · observed</div></div>
    <div className="metric-grid">
      <div className="metric"><div className="metric-label"><HardDrives size={13}/> Services</div><div className="metric-value">{healthy}/{total || "—"}</div><div className="metric-foot">Healthy internal dependencies</div></div>
      <div className="metric"><div className="metric-label"><SpinnerGap size={13}/> Running</div><div className="metric-value">{jobs.data?.jobs.filter((item) => item.status === "active").length ?? 0}</div><div className="metric-foot">Active ingestion jobs</div></div>
      <div className="metric"><div className="metric-label"><Check size={13}/> Complete</div><div className="metric-value">{jobs.data?.jobs.filter((item) => item.status === "completed").length ?? 0}</div><div className="metric-foot">Recent successful jobs</div></div>
      <div className="metric"><div className="metric-label"><ShareNetwork size={13}/> Agent usage</div><div className="metric-value">{recentTokens.toLocaleString()}</div><div className="metric-foot">tokens · ${recentCost.toFixed(4)} reported cost</div></div>
    </div>

    <div className="split-grid" style={{marginTop:28}}>
      <section>
        <div className="section-head" style={{marginTop:0}}><div className="section-title">Ingestion activity</div><button className="button ghost" onClick={() => jobs.refetch()}><ArrowClockwise size={11}/> Refresh</button></div>
        <div className="card list">
          ...
          : jobs.data?.jobs.length ? jobs.data.jobs.map((job) => <div className="list-row" key={job.id}>
            <div className="list-icon">{job.status === "completed" ? <Check size={15} color="var(--accent)"/> : job.status === "failed" ? <WarningCircle size={15} color="var(--danger)"/> : <Pulse size={15}/>}</div>
          ...
          </div>) : <div className="empty"><div><div className="empty-icon"><Clock size={18}/></div><strong>No ingestion activity yet</strong><span>Upload a file or add text from the overview.</span></div></div>}
        </div>
      </section>
      <section>
        ...
        <div className="card list">
          {health.data?.services.map((service) => <div className="list-row" key={service.service}>
            <div className="list-icon">{service.service === "postgres" ? <Database size={14}/> : service.service === "lightrag" ? <Graph size={14}/> : <ShareNetwork size={14}/>}</div>
          ...
          {!health.data && <div className="empty" style={{minHeight:180}}><SpinnerGap className="spin" size={20}/></div>}
        </div>
      </section>
    </div>
    ...
    <div className="card list">
      {researchRuns.length ? researchRuns.map((run) => <div className="list-row" key={`${run.kind}-${run.id}`}>
        <div className="list-icon">{run.kind === "workflow" ? <Clock size={14}/> : <ShareNetwork size={14}/>}</div>
      ...
      </div>) : <div className="empty" style={{minHeight:160}}><div><div className="empty-icon"><ShareNetwork size={18}/></div><strong>No research runs yet</strong><span>Ask a question or run a workflow to see token, cost, and latency telemetry.</span></div></div>}
    </div>
  </div>;
}
