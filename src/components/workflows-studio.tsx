...
import { CalendarClock, CheckCircle2, Clock3, Download, FileText, LoaderCircle, Pause, Play, Plus, Sparkles, Trash2, WandSparkles, XCircle } from "lucide-react";
...
export function WorkflowsStudio() {
  ...
  return <div className="page">
    <div className="eyebrow">Background intelligence</div>
    <h1 className="page-title">Put the company brain to work.</h1>
    <p className="page-description">Describe recurring research in plain language. Aperture creates bounded, read-only agents that run in the background and keep a complete evidence trail.</p>
    <form className="command workflow-command" onSubmit={submit}>
      <WandSparkles size={19} color="var(--lime)"/><input value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Every weekday at 9, summarize new launch risks…"/>
      <button className="button" disabled={!instruction.trim() || createDraft.isPending}>{createDraft.isPending ? <LoaderCircle className="spin" size={13}/> : <Sparkles size={13}/>}Draft workflow</button>
    </form>
    ...
    {draft && <section className="card workflow-draft">
      <div className="workflow-draft-head"><div><div className="eyebrow">Review before creating</div><h2>{draft.name}</h2></div><div style={{display:"flex",gap:7}}>{draft.output && draft.output !== "answer" && <span className="pill"><FileText size={10}/>{draft.output} artifact</span>}<span className="pill lime"><CalendarClock size={10}/>{draft.schedule ?? "Manual"}</span></div></div>
    ...
    <div className="workflow-grid">
      ...
      : workflows.data?.workflows.length ? workflows.data.workflows.map((workflow) => <article className="card workflow-card" key={workflow.id}>
        <div className="workflow-card-head"><div className={`workflow-state ${workflow.enabled ? "on" : ""}`}><Sparkles size={14}/></div><div><h3>{workflow.name}</h3><div className="list-meta">{workflow.schedule ?? "Manual"} · {workflow.timezone}{workflow.nextRunAt ? ` · next ${timeAgo(workflow.nextRunAt)}` : ""}</div></div><span className={`pill ${workflow.enabled ? "lime" : ""}`}>{workflow.status}</span></div>
        <p>{workflow.prompt}</p>
        <div className="run-list">{workflow.runs.length ? workflow.runs.slice(0,3).map((run) => <div className="run-row" key={run.id}>{run.status === "completed" ? <CheckCircle2 size={12} color="var(--lime)"/> : run.status === "failed" ? <XCircle size={12} color="var(--danger)"/> : <Clock3 size={12} color="var(--orange)"/>}<span>{run.status} · {timeAgo(run.createdAt)}</span>{run.latencyMs && <span>{(run.latencyMs/1000).toFixed(1)}s</span>}{run.artifact && <a href={`/api/workflows/runs/${run.id}/artifact`} className="pill lime" download><Download size={9}/>{run.artifact.format}</a>}</div>) : <div className="run-row"><Clock3 size={12}/><span>No runs yet</span></div>}</div>
        <div className="workflow-actions"><button className="button secondary" onClick={() => mutate.mutate({ id: workflow.id, method: "RUN" })}><Play size={12}/>Run now</button><button className="button ghost" onClick={() => mutate.mutate({ id: workflow.id, method: "PATCH", body: { enabled: !workflow.enabled } })}>{workflow.enabled ? <Pause size={12}/> : <CalendarClock size={12}/>}{workflow.enabled ? "Pause" : "Activate"}</button><button className="button ghost icon-button" aria-label={`Delete ${workflow.name}`} onClick={() => mutate.mutate({ id: workflow.id, method: "DELETE" })}><Trash2 size={12}/></button></div>
      </article>) : <div className="card empty" style={{gridColumn:"1/-1"}}><div><div className="empty-icon"><CalendarClock size={19}/></div><strong>No background workflows</strong><span>Describe one above. Nothing activates without your review.</span></div></div>}
    </div>
  </div>;
}
