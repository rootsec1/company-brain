...
test("primary research surfaces are available", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.getByRole("heading", { name: "Ask less. Know more." })).toBeVisible();
  await page.getByRole("link", { name: "Research" }).click();
  await expect(page.getByText("Research with the whole company in context.")).toBeVisible();
  await page.getByRole("link", { name: "Integrations" }).click();
  await expect(page.getByRole("heading", { name: "Bring your company with you." })).toBeVisible();
